...
class Renderer {
    ...
    struct {
        ...
        Vector max {std::numeric_limits<float>::min()};
    ...
    bool checkAABBcollision(const Vector& start, const Vector& ray) {
        Vector tMax(0), tMin(0);
        for (int i = 0; i < 3; ++i) {
            if (ray.get(i) == 0) {
                if (start.get(i) < AABB.min.get(i) || start.get(i) > AABB.max.get(i)) {
                    return false;
                } else {
                    tMin.set(i, std::numeric_limits<float>::min());
                    tMax.set(i, std::numeric_limits<float>::max());
                }
            } else {
                tMax.set(i, (AABB.max.get(i) - start.get(i)) / ray.get(i));
                tMin.set(i, (AABB.min.get(i) - start.get(i)) / ray.get(i));
            }
        }

        Vector t1 = {
            std::min(tMin.getX(), tMax.getX()), 
            std::min(tMin.getY(), tMax.getY()), 
            std::min(tMin.getZ(), tMax.getZ())
        };
        Vector t2 = {
            std::max(tMin.getX(), tMax.getX()), 
            std::max(tMin.getY(), tMax.getY()), 
            std::max(tMin.getZ(), tMax.getZ())
        };

        float tClose = std::max(std::max(t1.getX(), t1.getY()), t1.getZ());
        float tFar = std::min(std::min(t2.getX(), t2.getY()), t2.getZ());

        if(tClose > tFar || tFar < 0) return false;
...
public:
    ...
    void render(const std::string fileName) {
        ...
        for(unsigned int t = 0; t < numOfThreads; t++) {
            threads.emplace_back([this, imageBuffer]{
            while(1) {
            mtx.lock();
            if(buckets.size() == 0) {
                mtx.unlock();
            ...
            for (int rowIdx = bucketX; 
                ...
                for (int colIdx = bucketY; 
                    ...
                    if(checkAABBcollision(camera.getPosition(), camera.getImagePlane())) {
